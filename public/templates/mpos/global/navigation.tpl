    <hr/>
    <li class="icon-th-large"><a href="http://www.iwakuang.com" target="_blank">爱挖矿</a></li>
    ...
    {if $smarty.session.AUTHENTICATED|default: "0" == 1 && ($GLOBAL.userdata.is_admin == 1 || $GLOBAL.userdata.is_vip)}
      ...
      <ul class="toggle">
        ...
        <li class="icon-chart"><a href="{$smarty.server.PHP_SELF}?page=statistics&action=graphs">算力趋势图</a></li>
        {*<li class="icon-record"><a href="{$smarty.server.PHP_SELF}?page=statistics&action=round">周期</a></li>*}
        {*<li class="icon-search"><a href="{$smarty.server.PHP_SELF}?page=statistics&action=blockfinder">爆矿者</a></li>*}
        {*{if $GLOBAL.config.monitoring_uptimerobot_api_keys|default:"0"}<li class="icon-bell"><a href="{$smarty.server.PHP_SELF}?page=statistics&action=uptime">运行时间</a></li>{/if}*}
      </ul>
    {else}
      <h3>统计</h3>
